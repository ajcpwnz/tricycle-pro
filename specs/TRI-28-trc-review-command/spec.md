...
- **FR-017**: The command MUST support a `--post` flag that, after writing the local report, posts a condensed version (critical and warning findings only, no info) as a top-level PR comment. The same posting behavior MUST also be available durably via a `review.post_to_pr: true` setting in `tricycle.config.yml`.
- **FR-018**: When posting is triggered by `--post` (ad-hoc), the command MUST prompt the user for confirmation before posting and MUST NOT post if the user declines. When posting is triggered by `review.post_to_pr: true` (durable opt-in via version-controlled config), the command MUST NOT prompt — the user has already opted in by editing config.
