...
- `review.post_to_pr` — default `false`. When set to `true`, the command posts the condensed PR comment automatically without requiring `--post` on the command line, **and skips the interactive confirmation prompt** (the user has already opted in via durable, version-controlled config). The `--post` CLI flag remains available for ad-hoc one-off posting and is still confirmation-gated.
...
Trigger this step when **either** condition is true:

- The `--post` CLI flag was passed (ad-hoc opt-in, requires confirmation).
- `review.post_to_pr: true` in `tricycle.config.yml` (durable opt-in, no confirmation — the user has already opted in via version-controlled config).

If neither is true, skip step 7b entirely.
...
2. **Confirmation gate** — only when triggered by `--post` (not by config):
   ```
   Post review findings to PR #<N> as a comment? (y/N)
   ```
   Proceed only if the user answers `y` or `yes` (case-insensitive). Skip the prompt entirely when triggered by `review.post_to_pr: true`.
3. Run:
   ```bash
   gh pr comment "$PR_NUMBER" --body-file "$CONDENSED_TMP"
   ```
4. Print one of:
   - `Posted condensed review to PR #<N>.` (success — note whether triggered by `--post` or `review.post_to_pr`)
   - `Skipped posting (user declined).` (only possible from the `--post` path)
   - `Error: gh pr comment failed: <message>` (gh comment failed)
